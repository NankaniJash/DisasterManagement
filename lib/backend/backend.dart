...
Future maybeCreateUser(User user) async {
  final userRecord = UserPostsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await UserPostsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createUserPostsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      UserPostsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createUserPostsRecordData(email: email));
}
